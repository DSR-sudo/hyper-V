...
static void process_first_slat_hook()
{
	...
	// 输入：无；输出：Hook CR3 初始化；规则：仅首次执行；异常：不抛出。
 ...
 * @throws {无} 不抛出异常。
 * @example
 * slat::hook::set_up_entries();
 */
void slat::hook::set_up_entries()
{
	// 业务说明：分配一页内存作为条目池并构建空闲链表。
	// 输入：无；输出：available_hook_list_head；规则：按条目大小链入；异常：不抛出。
 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto ok = slat::hook::add(target_gpa, shadow_gpa);
 */
std::uint64_t slat::hook::add(const virtual_address_t target_guest_physical_address, const virtual_address_t shadow_guest_physical_address)
{
	...
	// 输入：目标/影子地址；输出：Hook 安装结果；规则：失败返回 0；异常：不抛出。
	hook_mutex.lock();
...
#else
	// 业务说明：AMD 平台设置执行权限并修复拆分页指令执行。
	// 输入：target_pte/hook_target_pte；输出：权限与拆分页状态更新；规则：目标页禁执行；异常：不抛出。
...
#else
	// 业务说明：AMD 平台还原执行权限并解除拆分页。
	// 输入：hook_entry；输出：PTE 权限恢复；规则：解除拆分页；异常：不抛出。
