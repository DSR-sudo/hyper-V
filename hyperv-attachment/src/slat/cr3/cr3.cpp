 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto cr3_value = slat::hyperv_cr3();
 */
cr3 slat::hyperv_cr3()
{
	// 业务说明：返回已缓存的 Hyper-V CR3。
	// 输入：无；输出：CR3 值；规则：返回当前缓存；异常：不抛出。
 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto cr3_value = slat::hook_cr3();
 */
cr3 slat::hook_cr3()
{
 ...
 * @throws {无} 不抛出异常。
 * @example
 * slat::set_up_hyperv_cr3();
 */
void slat::set_up_hyperv_cr3()
{
	// 业务说明：读取当前 SLAT CR3 作为 Hyper-V CR3。
	// 输入：无；输出：hyperv_slat_cr3 更新；规则：读取 VMCS；异常：不抛出。
...
{
	// 业务说明：复制 Hyper-V PML4 并构建 Hook CR3。
	// 输入：无；输出：hook_slat_cr3/hook_slat_pml4；规则：复制现有映射；异常：不抛出。
	...
	// 业务说明：确保 Hook SLAT 侧也隐藏堆页。
	// 输入：hook_cr3；输出：隐藏完成；规则：使用 Hook CR3；异常：不抛出。
