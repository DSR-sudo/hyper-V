...
std::uint64_t arch::get_vmexit_reason() { return vmread(VMCS_EXIT_REASON); }
...
std::uint8_t
arch::is_non_maskable_interrupt_exit(const std::uint64_t vmexit_reason) {
  if (vmexit_reason != VMX_EXIT_REASON_EXCEPTION_OR_NMI)
    return 0;
  const std::uint64_t raw = vmread(VMCS_VMEXIT_INTERRUPTION_INFORMATION);
  const vmexit_interrupt_information info = {
      .flags = static_cast<std::uint32_t>(raw)};
...
void arch::enable_mtf() {
  std::uint64_t controls =
      vmread(VMCS_CTRL_PROCESSOR_BASED_VM_EXECUTION_CONTROLS);
...
std::uint64_t arch::get_guest_rflags() { return vmread(VMCS_GUEST_RFLAGS); }
...
std::uint8_t arch::get_guest_cpl() {
  return vmread(VMCS_GUEST_CS_SELECTOR) & 3;
}

void arch::clear_guest_interruptibility() {
  // Manually define VMCS constant if missing in headers (0x4824)
  constexpr std::uint64_t VMCS_GUEST_INTERRUPTIBILITY_INFO_LOCAL = 0x00004824;
  vmwrite(VMCS_GUEST_INTERRUPTIBILITY_INFO_LOCAL, 0);
}
