...
/**
 * @description 创建并初始化 APIC 实例。
 * @param {void* const} allocation_ptr 预分配的内存指针。
 * @return {apic_t*} APIC 实例指针。
 * @throws {无} 不抛出异常。
 * @example
 * apic_t* apic = apic_t::create_instance(ptr);
 */
apic_t* apic_t::create_instance(void* const allocation_ptr)
{
	// 业务说明：在提供的内存空间上构造 xAPIC 或 x2APIC 实例。
	// 输入：allocation_ptr；输出：apic 实例；规则：按当前 CPU 模式选择子类；异常：不抛出。
	if (allocation_ptr == nullptr)
	{
		return nullptr;
	}

	void* const apic_allocation = allocation_ptr;
