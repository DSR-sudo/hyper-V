 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto addr = get_kernel_export(ctx, ntoskrnl_base, "ExAllocatePool");
 */
uint64_t get_kernel_export(context_t* ctx, const uint64_t module_base, const char* function_name)
{
    // 业务说明：通过来宾内存读取导出表并匹配函数名。
    // 输入：ctx/module_base/function_name；输出：函数地址；规则：仅支持非转发导出；异常：不抛出。
    if (!ctx || !module_base || !function_name) {
        return 0;
    }

    // Capture context CR3s
    const cr3 guest_cr3 = ctx->guest_cr3;
    const cr3 slat_cr3 = ctx->slat_cr3;
    ...
    for (uint32_t i = 0; i < export_dir.number_of_names; ++i) {
        ...
        if (str_compare_insensitive(current_name, function_name) == 0) {
            ...
            if (function_rva >= export_dir_entry.virtual_address &&
                ...
                logs::print(ctx->log_ctx, "[Loader] Warning: Forwarded export '%s' not supported\n", function_name);
                return 0;
            }
 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto ok = resolve_payload_imports(ctx, image, nt_base);
 */
bool resolve_payload_imports(context_t* ctx, void* payload_image, const uint64_t ntoskrnl_base)
{
    // 业务说明：遍历导入表，解析模块与函数并写入 IAT。
    // 输入：ctx/payload_image/ntoskrnl_base；输出：解析结果；规则：无法解析则失败；异常：不抛出。
    if (!ctx || !payload_image || !ntoskrnl_base) {
        logs::print(ctx->log_ctx, "[Loader] resolve_imports: Invalid parameters\n");
        return false;
    }
    ...
    if (!nt_headers) {
        logs::print(ctx->log_ctx, "[Loader] resolve_imports: Invalid PE headers\n");
        return false;
    }
    ...
    if (!import_dir.virtual_address) {
        ...
        logs::print(ctx->log_ctx, "[Loader] resolve_imports: No import table\n");
        return true;
    }
    ...
    while (import_descriptor->first_thunk) {
        ...
        logs::print(ctx->log_ctx, "[Loader] Resolving imports from: %s\n", module_name);
        ...
        if (str_compare_insensitive(module_name, "ntoskrnl.exe") == 0 ||
            ...
            if (find_guest_module(ctx, "HAL.dll", &hal_info) && hal_info.base_address) {
                resolve_module_base = hal_info.base_address;
                logs::print(ctx->log_ctx, "[Loader] HAL.dll found at 0x%p\n", resolve_module_base);
            } else {
                // HAL exports are often re-exported by ntoskrnl
                resolve_module_base = ntoskrnl_base;
                logs::print(ctx->log_ctx, "[Loader] HAL.dll fallback to ntoskrnl re-exports\n");
            }
            module_resolved = true;
        }
            ...
            if (find_guest_module(ctx, "NETIO.SYS", &netio_info) && netio_info.base_address) {
                resolve_module_base = netio_info.base_address;
                logs::print(ctx->log_ctx, "[Loader] NETIO.SYS found at 0x%p\n", resolve_module_base);
                module_resolved = true;
            } else {
                logs::print(ctx->log_ctx, "[Loader] ERROR: NETIO.SYS not found - cannot resolve imports\n");
                return false;
            }
        }
            ...
            if (find_guest_module(ctx, "fwpkclnt.sys", &fwp_info) && fwp_info.base_address) {
                resolve_module_base = fwp_info.base_address;
                logs::print(ctx->log_ctx, "[Loader] fwpkclnt.sys found at 0x%p\n", resolve_module_base);
                module_resolved = true;
            } else {
                logs::print(ctx->log_ctx, "[Loader] ERROR: fwpkclnt.sys not found - cannot resolve imports\n");
                return false;
            }
        }
        else if (str_compare_insensitive(module_name, "NDIS.SYS") == 0) {
            guest_module_info_t ndis_info = {};
            if (find_guest_module(ctx, "NDIS.SYS", &ndis_info) && ndis_info.base_address) {
                resolve_module_base = ndis_info.base_address;
                logs::print(ctx->log_ctx, "[Loader] NDIS.SYS found at 0x%p\n", resolve_module_base);
                module_resolved = true;
            } else {
                logs::print(ctx->log_ctx, "[Loader] ERROR: NDIS.SYS not found - cannot resolve imports\n");
                return false;
            }
        }
        else {
            ...
            if (find_guest_module(ctx, module_name, &unknown_info) && unknown_info.base_address) {
                resolve_module_base = unknown_info.base_address;
                logs::print(ctx->log_ctx, "[Loader] %s found at 0x%p\n", module_name, resolve_module_base);
                module_resolved = true;
            } else {
                logs::print(ctx->log_ctx, "[Loader] ERROR: Unknown module %s not found\n", module_name);
                return false;
            }
        }
        ...
        while (original_first_thunk->u1.address_of_data) {
            ...
            if (original_first_thunk->u1.ordinal & (1ULL << 63)) {
                logs::print(ctx->log_ctx, "[Loader] Error: Import by ordinal not supported\n");
                return false;
            }
            ...
            uint64_t function_address = get_kernel_export(ctx, resolve_module_base, func_name);
            ...
            if (!function_address && resolve_module_base != ntoskrnl_base) {
                ...
                if (str_compare_insensitive(module_name, "HAL.dll") == 0) {
                    function_address = get_kernel_export(ctx, ntoskrnl_base, func_name);
                    if (function_address) {
                        logs::print(ctx->log_ctx, "[Loader] %s resolved via ntoskrnl re-export\n", func_name);
                    }
                }
            }
            ...
            if (!function_address) {
                ...
                logs::print(ctx->log_ctx, "[Loader] ERROR: Failed to resolve import: %s!%s\n", 
