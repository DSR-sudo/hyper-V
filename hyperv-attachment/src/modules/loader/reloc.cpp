 ...
 * @throws {无} 不抛出异常。
 * @example
 * const auto ok = loader::apply_relocations(ctx, image_base, target_va);
 */
bool apply_relocations(context_t* ctx, void* payload_image, const uint64_t target_va)
{
    ...
    if (!nt_headers) {
        logs::print(ctx->log_ctx, "[Loader] apply_relocations: Invalid PE headers\n");
        return false;
    }
    ...
    if (!reloc_dir.virtual_address || !reloc_dir.size) {
        ...
        logs::print(ctx->log_ctx, "[Loader] apply_relocations: No relocation table (PIC code?)\n");
        return true;
    }
    ...
    if (delta == 0) {
        logs::print(ctx->log_ctx, "[Loader] apply_relocations: Delta is 0, no relocation needed\n");
        return true;
    }

    logs::print(ctx->log_ctx, "[Loader] Applying relocations: Original=0x%p, Target=0x%p, Delta=0x%p\n",
        ...
        for (uint32_t i = 0; i < entry_count; ++i) {
            ...
            if (type == IMAGE_REL_BASED_DIR64) {
                ...
                *target += delta;
                ...
                logs::print(ctx->log_ctx, "[Loader] Warning: Unsupported relocation type %d at offset 0x%x\n",
                    type, offset);
            }
        }
    ...
    logs::print(ctx->log_ctx, "[Loader] Applied %d DIR64 relocations\n", total_relocs_applied);
    return true;
}
